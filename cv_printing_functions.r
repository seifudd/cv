...
create_CV_object <- function(data_location,
  ...
  if (is_google_sheets_location) {
    ...
    cv$publication_info <- read_gsheet(sheet_id = "publication_info")
  } else {
    ...
    cv$publication_info <- readr::read_csv(paste0(data_location, "publication_info.csv"), skip = 1)
  }
...
#' @description Publication information section with icons
print_publications_info <- function(cv) {
  glue::glue_data(
    cv$publication_info,
    "- <i class='fa fa-{icon}'></i> {publications}"
  ) |> print()
